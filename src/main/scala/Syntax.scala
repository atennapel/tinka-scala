...
object Syntax:
  ...
  enum Tm:
    ...
    override def toString: String = this match
      ...
      case Lam(x, Impl(Unif), b)           => s"(\\{$x}. $b)"
      case Lam(x, Impl(Inst), b)           => s"(\\{{$x}}. $b)"
      case App(fn, arg, Expl)              => s"($fn $arg)"
      case App(fn, arg, Impl(Unif))        => s"($fn {$arg})"
      case App(fn, arg, Impl(Inst))        => s"($fn {{$arg}})"
      case AppLvl(l, r)                    => s"($l <$r>)"
      case Pi(x, Expl, t, u1, b, u2)       => s"(($x : $t) ->{$u1}{$u2} $b)"
      case Pi(x, Impl(Unif), t, u1, b, u2) => s"({$x : $t} ->{$u1}{$u2} $b)"
      case Pi(x, Impl(Inst), t, u1, b, u2) => s"({{$x : $t}} ->{$u1}{$u2} $b)"
